...
const DashboardActivity = () => {
    ...
    return (
        <div className="relative bg-surface min-h-screen text-theme overflow-hidden">
            <div className="md:p-5">
                {/* Search bar */}
                <div className="flex flex-row md:justify-between md:items-center bg-card shadow p-2 rounded mb-6 gap-3 mt-5">
                    <div className="relative flex-1">
                        <Search className="absolute left-2 md:top-2 top-1 h-5 w-5 text-gray-500" />
                        <input
                            type="text"
                            placeholder="Search logs"
                            className="pl-10 md:pr-2 md:py-2 pr-2 py-1 rounded-md text-base w-full focus:outline-none focus:ring-0 text-theme bg-surface"
                        />
                    </div>
                    <div className="hidden md:block w-px h-6 border-theme"></div>
                    <span className="hidden md:flex text-secondary font-medium text-sm md:text-base">
                        superadmin
                    </span>
                </div>
                ...
                <div className="bg-card shadow rounded overflow-x-auto">
                    <table className="table-auto w-full border-collapse min-w-[600px] text-theme">
                        <thead>
                            <tr className="bg-[var(--color-primary)] text-white text-left text-sm md:text-base">
                        ...
                        <tbody>
                            {logs.map((item, index) => (
                                <tr
                                    key={item.id}
                                    className={`border-b border-theme text-sm md:text-base ${index % 2 === 0 ? "bg-surface" : "bg-card"
                                        }`}
                                >
