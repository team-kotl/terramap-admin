...
const Download = () => {
  ...
  const handleApprove = (reqId) => {
    console.log("Approved request with ID:", reqId);
    alert(`✅ Request ${reqId} approved`);
    // TODO: call backend API to approve
  };

  const handleReject = (reqId) => {
    console.log("Rejected request with ID:", reqId);
          ...
          <table className="table-auto w-full border-collapse">
            <thead>
              <tr className="bg-green-800 text-white text-left text-sm md:text-base">
                <th className="px-4 py-3 min-w-[180px]">Timestamp</th>
                ...
                <th className="px-4 py-3 min-w-[160px]">Affiliation</th>
                <th className="px-4 py-3 min-w-[220px]">Purpose</th>
                <th className="px-4 py-3 min-w-[160px] text-center">Action</th>
              </tr>
            </thead>
            ...
            <tbody>
              {filteredDownloads.map((item, index) => (
                ...
                >
                  <td className="px-4 py-3">{formatTimestamp(item.timestamp)}</td>
                  <td className="px-4 py-3">{item.reqId}</td>
                  <td className="px-4 py-3">{item.aoi}</td>
                  <td className="px-4 py-3">{item.name}</td>
                  <td className="px-4 py-3">{item.email}</td>
                  <td className="px-4 py-3">{item.address}</td>
                  <td className="px-4 py-3">{item.affiliation}</td>
...
<td className="px-4 py-3 flex gap-1 justify-center">
  <button
    ...
    className="bg-green-700 hover:bg-green-800 text-white p-1 rounded-full flex items-center justify-center"
  >
    <Check size={16} />
  </button>
  <button
    onClick={() => handleReject(item.reqId)}
    className="bg-red-700 hover:bg-red-800 text-white p-1 rounded-full flex items-center justify-center"
  >
    <X size={16} />
  </button>
</td>
          ...
          {filteredDownloads.length === 0 && (
            <p className="p-4 text-gray-500 text-center">No records found.</p>
          )}
        </div>
      </div>
    </div>
